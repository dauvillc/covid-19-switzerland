...
def compute_aggregated_new_infections(avg_contact_matrix, proba):
    """
    ...
    :param proba: vector of length n_activity_types giving the probability
        of transmission for each activity type.
    :return: the aggregated dI as an array of shape (n_age_groups).
    """
    avg_contact_matrix = avg_contact_matrix * proba
    return np.sum(avg_contact_matrix, axis=1)


def average_contact_matrix(contact_matrices, proba):
    """
    ...
    :param proba: vector of length n_activity_types giving the
